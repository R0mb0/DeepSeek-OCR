...
from typing import List, Literal, Optional, Set, Tuple, TypedDict, Union, Any
...
# SamplingMetadata fallback
try:
    from vllm.model_executor import SamplingMetadata
except Exception:
    SamplingMetadata = Any  # type: ignore
...
# Import configuration and ensure get_tokenizer is available
from config import IMAGE_SIZE, BASE_SIZE, CROP_MODE, PRINT_NUM_VIS_TOKENS, PROMPT, get_tokenizer
...
class DeepseekOCRProcessingInfo(BaseProcessingInfo):
    ...
    def get_hf_processor(self, **kwargs: object):
        # Provide a concrete tokenizer to the processor construction when possible.
        try:
            ...
            if tokenizer_instance is not None:
                return self.ctx.get_hf_processor(DeepseekOCRProcessor, tokenizer=tokenizer_instance, **kwargs)
        except Exception:
            pass
    ...
    def get_tokenizer(self):
        """
        Return a concrete tokenizer instance for use inside EngineCore.
        vllm expects self.info.get_tokenizer() to return a tokenizer object
        with a decode() method during profiling / prompt updates.
        We prefer the tokenizer created in config (get_tokenizer()).
        """
        try:
            t = get_tokenizer()
            if t is not None:
            ...
            pass
        # Fallback: try to obtain via ctx if available (best-effort)
        try:
    ...
    def get_num_image_tokens(self,
        ...
        # Use configuration constants to compute token counts for images
...
class DeepseekOCRMultiModalProcessor(BaseMultiModalProcessor[DeepseekOCRProcessingInfo]):
    def _call_hf_processor(self,
                          prompt: str,
                          mm_data: Mapping[str, object],
                          mm_kwargs: Mapping[str, object],
                          *args,
                          **kwargs) -> BatchFeature:
        # Accept and forward extra kwargs (tok_kwargs / tokenization_kwargs) new vllm may pass
        tok_kw = {}
        for name in ("tok_kwargs", "tokenization_kwargs", "tokenizer_kwargs"):
            if name in kwargs:
                maybe_map = kwargs.pop(name)
                if isinstance(maybe_map, dict):
                    tok_kw.update(maybe_map)

        proc_kwargs = {}
        if mm_kwargs:
            proc_kwargs.update(mm_kwargs)
        for k, v in list(kwargs.items()):
            if k != "prompt":
                proc_kwargs.setdefault(k, v)

        if mm_data:
            try:
                processor = self.info.get_hf_processor(**proc_kwargs)
            except Exception:
                processor = self.info.get_hf_processor()
            inputs = dict(prompt=prompt, **mm_data)
            try:
                if tok_kw:
                    processed_outputs = self.info.ctx.call_hf_processor(processor, inputs, proc_kwargs, **tok_kw)
                else:
                    processed_outputs = self.info.ctx.call_hf_processor(processor, inputs, proc_kwargs)
            except TypeError:
                processed_outputs = self.info.ctx.call_hf_processor(processor, inputs, proc_kwargs)
        else:
            ...
            if tokenizer is None:
                try:
                    ...
                    tokenizer = None
            if tokenizer is None:
                raise RuntimeError("No tokenizer available to process text prompt")
            try:
                processed_outputs = tokenizer(prompt, add_special_tokens=True, return_tensors="pt", **tok_kw)
            except TypeError:
    ...
    def _get_prompt_updates(self, mm_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], out_mm_kwargs: MultiModalKwargs) -> Sequence[PromptUpdate]:
        ...
        def get_replacement_deepseek_vl2(item_idx: int):
            ...
            if isinstance(images, ImageEmbeddingItems):
                ...
                num_image_tokens = self.info.get_num_image_tokens(image_width=width, image_height=height, cropping=CROP_MODE)
            return [image_token_id] * num_image_tokens

        return [PromptReplacement(modality="image", target=[image_token_id], replacement=get_replacement_deepseek_vl2)]

    def _cached_apply_hf_processor(self, prompt: Union[str, list[int]], mm_data_items: MultiModalDataItems, hf_processor_mm_kwargs: Mapping[str, object], *args, **kwargs) -> tuple[list[int], MultiModalKwargs, bool]:
        if mm_data_items.get_count("image", strict=False) > 2:
            return self._apply_hf_processor_main(prompt=prompt, mm_items=mm_data_items, hf_processor_mm_kwargs=hf_processor_mm_kwargs, enable_hf_prompt_update=True, *args, **kwargs)
        return super()._cached_apply_hf_processor(prompt=prompt, mm_data_items=mm_data_items, hf_processor_mm_kwargs=hf_processor_mm_kwargs, *args, **kwargs)


@MULTIMODAL_REGISTRY.register_processor(DeepseekOCRMultiModalProcessor, info=DeepseekOCRProcessingInfo, dummy_inputs=DeepseekOCRDummyInputsBuilder)
class DeepseekOCRForCausalLM(nn.Module, SupportsMultiModal, SupportsPP):
    ...
    def _pixel_values_to_embedding(self, pixel_values: torch.Tensor, images_crop: torch.Tensor, images_spatial_crop: torch.Tensor) -> NestedTensors:
    ...
    def _process_image_input(self, image_input) -> torch.Tensor:
        ...
        vision_features = self._pixel_values_to_embedding(pixel_values=pixel_values, images_crop=images_crop, images_spatial_crop=images_spatial_crop)
